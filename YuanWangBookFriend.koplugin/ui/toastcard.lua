...
function ToastCard:open(plugin, opts)
    ...
    --[[--
    引文的唯一出口：保留原文段落结构（换行留着，连续 3 个以上换行压成 2 个）。

    与 `ui/chatdialog.lua` 同一口径，理由也相同：以前这里用 `selected_clean`
    （`sanitizeForDisplay` 的产物，而它的 `%c` **包含 `\n`**，实测
    `("a\nb"):gsub("%c","")=="ab"`），**存进历史的那一份从一开始就没有换行**，
    于是收藏详情页 / 导出 /「查看选中原文」三处看到的永远是一坨——
    展示层再怎么改都救不回来。落点必须在写入侧。
    --]]
                    ...
                    },
                    {
                        ...
                        callback = function()
                            ...
                            UIManager:show(TextViewer:new{
                                title = _("选中的原文"),
                                -- 与存进历史的那一份同源：这里看到的就该是原文的段落结构；
                                -- 排版（首行缩进 2 字）只加在显示层，见 Util.quoteBlock
                ...
                {
                    {
                        ...
                        callback = function()
                            ...
                            Asker:submitAsync(plugin, {
                                ...
                                title = _("远望书友-轻问回复"),
                                -- 存进历史的就是带换行的那一份（见 selected_kept 的注释）
                                selected = selected_kept,
                                page_text = opts.page_text,
                                book_fp = opts.book_fp,
